...
static HW_BOOL sk_skip_input_data(HwJpegInputInfo* hwInfo, long num_bytes) {
    sk_hw_jpeg_source_mgr*  src = (sk_hw_jpeg_source_mgr*)hwInfo->streamCtl.inStream;
LOGYC("skip num_bytes: %d, cur offset : %d\n", num_bytes, src->cur_offset_instream);
	if (num_bytes > (long)src->bytes_in_buffer) {
		...
		if(hwInfo->justcaloutwh && (bytesToSkip + src->cur_offset_instream > LARGEST_LENGTH)){
			LOGYC("skip over Largest length.");
			return false;
		}
        while (bytesToSkip > 0) {
            ...
            if (bytes <= 0 || bytes > bytesToSkip) {
				printf("failure to skip request %ld returned %ld\n", bytesToSkip, bytes);
                return false;
            }
...
LOGYC("skip finish, cur offset : %d\n", src->cur_offset_instream);
	return true;
}
...
static HW_BOOL sk_fill_input_buffer(HwJpegInputInfo* hwInfo) {
    sk_hw_jpeg_source_mgr* src = (sk_hw_jpeg_source_mgr*)hwInfo->streamCtl.inStream;
LOGYC("fill input buffer start cur_offset: %d\n", src->cur_offset_instream);
	size_t bytes = 0;
	if(hwInfo->justcaloutwh && (src->cur_offset_instream + sk_hw_jpeg_source_mgr::kBufferSize > LARGEST_LENGTH)){
		if(LARGEST_LENGTH <= src->cur_offset_instream){
			LOGYC("check here, LARGEST_LENGTH <= src->cur_offset_instream");
			return false;
		}
...
LOGYC("<%s>_%d buf=%p, bytes = %d \n",__func__,__LINE__,src->fBuffer, bytes);
...
LOGYC("fill input buffer finish, bytes: %d, cur_offset: %d\n", bytes, src->cur_offset_instream);
    return true;
}
...
static HW_BOOL sk_resync_to_restart(HwJpegInputInfo* hwInfo) {
    ...
    if (!src->fStream->rewind()) {
        LOGYC("____________failure to rewind\n");
        return false;
    }
	src->cur_offset_instream = 0;
    src->next_input_byte = (const unsigned char*)src->fBuffer;
...
static int sk_fill_buffer(HwJpegInputInfo* hwInfo, void * des, VPUMemLinear_t *newVpuMem, int w, int h){
	...
	if(des == NULL/* || (src->fDecoder != NULL && src->fDecoder->shouldCancelDecode())*/) {
		LOGYC("return -1 at start in fill buffer.\n");
		return -1;
	}
...
LOGYC("bytes: %d, hwInfo->streamCtl.wholeStreamLength: %d,  *bytesHasIn: %d \n", bytes, hwInfo->streamCtl.wholeStreamLength, *bytesHasIn);
		if(bytes <= 0){
...
LOGYC("<%s>_%d    destination = %p  bytes = %d   REAL rdbytes: %d \n", __func__, __LINE__, destination, bytes, rdBytes);
    if (bytes > 0) {
	...
	if(rdBytes < bytes){//recompute whole length
		...
		WHREDLOG("REAL whole stream length: %d\n", rdBytes);
	}
HANDLE:
	//if stream getlength is wrong, will go follow codes
	unsigned char* buf = src->fBuffer;
	size_t size_ = sk_hw_jpeg_source_mgr::kBufferSize; 
	rdBytes = stream->read(buf, size_);
LOGYC("<%s>_%d buf=%p,kBufferSize=%d,size_=%d reReadbyte is : %d \n",__func__,__LINE__,buf,sk_hw_jpeg_source_mgr::kBufferSize,size_,rdBytes);
#ifndef UNLIKELY
...
#endif
	if(rdBytes > 0){
		printf("all right , we need relloc a vpu mem, first readbyte is : %d \n", rdBytes);
		...
		if(rdBytes < sk_hw_jpeg_source_mgr::kBufferSize){
			...
			LOGYC("11 tmp: %d\n", tmp);
			LOGYC("read is filling buf, and input wh: %d,%d \n", w , h);
			if(LIKELY(w > 0 && h > 0)){
				if(LIKELY(mallocSize < w*h)){
					mallocSize = w*h;
				}else{
					LOGYC("readed bytes: %d, input w: %d, input h: %d\n", mallocSize, w, h);
					LOGYC("net stream, now dicide the mallocSize to four times of readed bytes.\n");
					mallocSize *= 4;
				}
			}else{
				LOGYC("net stream, and not get w or h before, now dicide the mallocSize to four times of readed bytes.\n");
				mallocSize *= 4;
			}
			mallocSize = (mallocSize + 255)&(~255);
			if(mallocSize < JPEG_INPUT_BUFFER) {
			...
			if(LIKELY(tmpVpuMem.vir_addr != NULL)){
				...
				if(rdBytes > 0){
					tmp += rdBytes;
					LOGYC("22 tmp: %d, rdBytes: %d\n", tmp, rdBytes);
					do{
						if(UNLIKELY(tmp >= mallocSize)){
							LOGYC("unenough space, we need relloc a vpumem to fill data\n");
							mallocSize *= 2;
							LOGYC("second real mallocsize: %d\n", mallocSize);
...
static HW_BOOL sk_read_1_byte(HwJpegInputInfo* hwInfo, unsigned char * ch){
	...
	if(src->bytes_in_buffer == 0){
		if(!sk_fill_input_buffer(hwInfo)) {
			LOGYC("fill_input_buffer fail, may be at the end of the stream.");
			return false;
		}
	}
	src->bytes_in_buffer--;
...
SkJpegVPUMemStream::SkJpegVPUMemStream(SkStream * stream, size_t *len){
	...
	LOGYC("SkJpegVPUMemStream size: %d ", mallocSize);
	...
	if(vpuMem.vir_addr != NULL)
	{
		...
		while(bytesInStream < size && error == false){
			...
			LOGYC("meet mark : 0x%x, readBytes: %d", twoBytes, readBytes);
			error = (readBytes<=0);
			if(error){
				break;
			}
		...
		if(error == false && bytesInStream > 0){
			LOGYC("bytesInStream is : %d, size is : %d", bytesInStream, size);
			...
			LOGYC("read all to vpumemstream fail.");
