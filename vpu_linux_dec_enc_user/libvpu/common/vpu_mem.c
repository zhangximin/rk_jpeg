...
#define VPM_DEBUG(fmt, args...) printf(fmt, ## args)
#else
#define VPM_DEBUG(fmt, args...) /* not debugging: nothing */
#endif

#ifdef _VPU_MEM_ERROR
#define VPM_ERROR(fmt, args...) printf(fmt, ## args)
#else
...
static RK_S32 vpu_ion_link()
{
    ...
    if (err) {
        VPM_ERROR(" <%s>_%d open ion_mem failed\n", __func__, __LINE__);
        goto OPEN_ERR;
    }
    ...
    do {
        ...
        if (pool) {
            ...
            if (pool) {
                ...
                printf("VPUMemPoolSet: add pfn %d count %d", pfn, count);
            } else {
...
RK_S32 VPUMemPoolUnset(RK_U32 size, RK_U32 count)
{
    ...
    do {
        ...
        vpu_mem_pool * pool = find_pool_by_pfn(pfn);
        if (pool) {
            printf("VPUMemPoolUnset: del pfn %d count %d", pfn, count);
            pool->count -= count;
            if (pool->count <= 0) {
...
static RK_S32 vpu_lock_init(pthread_mutex_t *mutex)
{
    ...
    if (0 == pthread_mutexattr_init(&attribute)) {
        if (0 == pthread_mutexattr_settype(&attribute, PTHREAD_MUTEX_RECURSIVE_NP)) {
...
RK_HANDLE VPUMemAllocatorCreate(RK_U32 maxSize)
{
    ...
    do {
        ...
        if (vpu_mem_status) {
            ...
            if (err) {
                VPM_ERROR(" <%s>_%d open ion_mem failed ret %d\n", __func__, __LINE__, err);
                break;
            } else {
...
{
    if (NULL == p) {
        printf("VPUMemMalloc: NULL ptr for handle");
        return INVALID_NULL_PTR;
    }
    *p = NULL;
    if (check_allocator(allocator)) {
        printf("VPUMemMalloc: invalid allocator %p", allocator);
...
{
    if (NULL == p) {
        printf("VPUMemImport: NULL ptr for handle");
        return INVALID_NULL_PTR;
    }
    *p = NULL;
    if (check_allocator(allocator)) {
        printf("VPUMemImport: invalid allocator %p", allocator);
        return INVALID_ALLOCATOR;
    }
...
{
    if (NULL == p) {
        printf("VPUMemFree: NULL ptr for handle");
        return INVALID_NULL_PTR;
    }
    if (check_mem_hnd(p)) {
        printf("VPUMemFree: invalid mem hnd %p", p);
        return INVALID_MEM_HND;
    }
    VPUMemSlot *pSlot = (VPUMemSlot *)p;
    if (check_allocator(pSlot->allocator)) {
        printf("VPUMemFree: invalid allocator %p", pSlot->allocator);
        return INVALID_ALLOCATOR;
    }
...
RK_S32 VPUMemShare(VPUMemHnd *p, RK_S32 *mem_fd)
{
    if (NULL == p || NULL == mem_fd) {
        printf("VPUMemShare: NULL ptr for handle");
        return INVALID_NULL_PTR;
    }
    if (check_mem_hnd(p)) {
        printf("VPUMemShare: invalid mem hnd %p", p);
        return INVALID_MEM_HND;
    }
    VPUMemSlot *pSlot = (VPUMemSlot *)p;
    if (check_allocator(pSlot->allocator)) {
        printf("VPUMemShare: invalid allocator %p", pSlot->allocator);
        return INVALID_ALLOCATOR;
    }
    VPUMemAllocator *pAlloc = (VPUMemAllocator *)pSlot->allocator;
    RK_S32 err;
...
{
    if (NULL == p) {
        printf("VPUMemCache: NULL ptr for handle");
        return INVALID_NULL_PTR;
    }
    if (check_mem_hnd(p)) {
        printf("VPUMemCache: invalid mem hnd %p", p);
        return INVALID_MEM_HND;
    }
    VPUMemSlot *pSlot = (VPUMemSlot *)p;
    if (check_allocator(pSlot->allocator)) {
        printf("VPUMemCache: invalid allocator %p", pSlot->allocator);
        return INVALID_ALLOCATOR;
    }
    VPUMemAllocator *pAlloc = (VPUMemAllocator *)pSlot->allocator;
    RK_S32 err;
