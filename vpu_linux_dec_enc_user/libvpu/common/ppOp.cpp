    ...
    : vpuType(VPU_TYPE_BUTT), ptr(NULL), start(NULL)
{
    ...
    default : {
        printf("invalid vpu client type: %d", type);
    } break;
    }
    if (NULL != ptr) {
        vpuType = type;
        ...
        printf("failed to malloc resource for VPUReg");
        size = 0;
    }
}
...
void VPUReg::SetRegisterFile(RK_U32 id, RK_U32 value)
{
    if (vpuType >= VPU_TYPE_BUTT) {
        printf("failed to SetRegisterFile to invalid VPUReg");
        return ;
    }
...
{
    if (vpuType >= VPU_TYPE_BUTT) {
        printf("failed to GetRegisterFile from invalid VPUReg");
        return 0;
    }
    RK_U32 tmp = start[hwDecRegSpec[id][0]];
...
status_t ppOpInit(PP_OP_HANDLE *hnd, PP_OPERATION *init)
{
    if (NULL == hnd || NULL == init) {
        printf("invalid arg reg: %p init: %p", hnd, init);
        return BAD_VALUE;
    }
    VPUReg *reg = new VPUReg(VPU_PP);
    PP_INTERNAL *p = (PP_INTERNAL *)malloc(sizeof(PP_INTERNAL));
    if (NULL == p || NULL == reg) {
        printf("can not malloc resource for pp operation");
        if (p) free(p);
        if (reg) delete(reg);
        return NO_MEMORY;
    }
    ...
    if (ppSetSrcFormat(reg, init->srcFormat) || ppSetDstFormat(reg, init->dstFormat)) {
        printf("invalid format src: %d, dst %d", init->srcFormat, init->dstFormat);
        if (p) free(p);
        if (reg) free(reg);
        return BAD_VALUE;
    }
...
status_t ppOpSet(PP_OP_HANDLE hnd, PP_SET_OPT opt, uint32_t val)
{
    if (NULL == hnd || ppOptCheck(opt)) {
        printf("invalid arg hnd: %p opt: %d", hnd, opt);
        return BAD_VALUE;
    }
...
{
    if (NULL == p) {
        printf("invalid NULL hnd");
        return BAD_VALUE;
    }
    if (p->checkSum != (uint32_t)p) {
        printf("invalid hnd: %p checkSum 0x%.8x", p, p->checkSum);
        return BAD_VALUE;
    }
    if (p->vpuFd <= 0) {
        printf("invalid vpu client handle: %d", p->vpuFd);
        return BAD_VALUE;
    }
    if (NULL == p->reg) {
        printf("invalid register set: %p", p->reg);
        return BAD_VALUE;
    }
    return OK;
}
...
status_t ppOpPerform(PP_OP_HANDLE hnd)
{
    ...
    for (int i=0; i < VPU_REG_NUM_PP; i++) {
        printf("send reg[%.2d] val: 0x%.8x", i, ptr[i]);
    }
#endif
...
status_t ppOpSync(PP_OP_HANDLE hnd)
{
    ...
    RK_U32 *ptr = p->reg->addr();
    for (int i=0; i < VPU_REG_NUM_PP; i++) {
        printf("recv reg[%.2d] val: 0x%.8x", i, ptr[i]);
    }
#endif
    if (ret) return ret;
...
int main()
{
    ...
    if (ret) {
        printf("failed to malloc vpu_mem");
        goto end;
    }
    if (vpuFd < 0) {
        printf("failed to open vpu client");
        goto end;
    }
    ...
    if (ret) {
        printf("ppOpInit failed");
        hnd = NULL;
        goto end;
    }
    ret = android::ppOpPerform(hnd);
    if (ret) {
        printf("ppOpPerform failed");
    }
    ret = android::ppOpSync(hnd);
    if (ret) {
        printf("ppOpSync failed");
    }
