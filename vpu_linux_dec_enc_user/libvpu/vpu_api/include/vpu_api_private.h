...
public:
    ...
    static int32_t on2_api_init(VPU_API *on2_api, uint32_t video_coding_type);

    int32_t video_fill_hdr(uint8_t *dst,uint8_t *src, uint32_t size,
            int64_t time, uint32_t type, uint32_t num);

    int32_t m2v_fill_hdr(uint8_t *dst, uint8_t *src, uint32_t size,
    ...
    On2CodecConfigure_t* getStagefrightCfgByCoding(CODEC_TYPE codec_type, uint32_t video_coding_type);
    On2CodecConfigure_t* getStagefrightCfgByName(CODEC_TYPE codec_type, uint8_t* name);
