                              ...
                              const H264EncRateCtrl * pRateCtrl)
{
...

    {
		...
		if(pEncInst->asic.regs.vpuid == 0x30)
		{
	...
	if(pEncInst->asic.regs.vpuid == 0x30)
		(void) H264InitRc_new(rc);  /* new parameters checked already */
	else
                             ...
                             H264EncOut * pEncOut)
{
	...
	if(pEncInst->asic.regs.vpuid == 0x30)
	    /* Rate control */
	    H264BeforePicRc_new(&pEncInst->rateControl, pEncIn->timeIncrement,
                    pSlice->sliceType);
	else
...
//    while(1);
    ...
    /* Rate control action after vop */
    {
		...
		if(pEncInst->asic.regs.vpuid == 0x30)
	        stat = H264AfterPicRc_new(&pEncInst->rateControl, regs->rlcCount,
...
void H264GetNalUnitSizes(const h264Instance_s * pEncInst, u32 * pNaluSizeBuf)
{
    ...
    pTmp = (u32 *) pEncInst->asic.sizeTbl.nal.vir_addr;
