...
#define DIV_CEIL(a,b) (((a)+(b)-1)/(b))
    ...
    ALOGV("***** ref buffer mem model trace *****\n");
    ALOGV("latency             = %7d clk\n", pRefbu->currMemModel.latency );
    ALOGV("sequential          = %7d clk\n", pRefbu->currMemModel.nonseq );
    ALOGV("non-sequential      = %7d clk\n", pRefbu->currMemModel.seq );

    ALOGV("latency (mb)        = %7d\n", pRefbu->memAccessStats.latency );
    ALOGV("sequential (mb)     = %7d\n", pRefbu->memAccessStats.nonseq );
    ALOGV("non-sequential (mb) = %7d\n", pRefbu->memAccessStats.seq );

    ALOGV("bus-width           = %7d\n", busWidth );

    ALOGV("buffering cycles    = %7d\n", pRefbu->numCyclesForBuffering );
    ALOGV("buffer penalty      = %7d\n", pRefbu->bufferPenalty );
    ALOGV("avg cycles per mb   = %7d\n", pRefbu->avgCyclesPerMb );
...
#ifdef REFBUFFER_TRACE
    ALOGV("***** ref buffer threshold trace *****\n");
#endif /* REFBUFFER_TRACE */
...
#ifdef REFBUFFER_TRACE
        ALOGV("predicted misses    = %7d\n", predMiss );
        ALOGV("data excess %%       = %7d\n", pRefbu->dataExcessMaxPct );
        ALOGV("divisor             = %7d\n", divisor );
#endif /* REFBUFFER_TRACE */
...
#ifdef REFBUFFER_TRACE
    ALOGV("***** ref buffer initialized to %dx%d *****\n",
...
#ifdef REFBUFFER_TRACE
    ALOGV("***** ref buffer algorithm trace *****\n");
    ALOGV("predicted coverage  = %7d\n", cov );
    ALOGV("coverage history    = %7d%7d%7d\n",
        pRefbu->coverage[0],
        pRefbu->coverage[1],
        pRefbu->coverage[2] );
    ALOGV("enable              = %d (%8d<%8d)\n", enable, numCyclesForBuffering, tmp );
#endif /* REFBUFFER_TRACE */
...
#ifdef REFBUFFER_TRACE
    ALOGV("***** ref buffer mv statistics trace *****\n");
    ALOGV("intra blocks        = %7d\n", numIntraBlk );
    ALOGV("top fields          = %7d\n", topFldCnt );
    ALOGV("bottom fields       = %7d\n", botFldCnt );
#endif /* REFBUFFER_TRACE */
