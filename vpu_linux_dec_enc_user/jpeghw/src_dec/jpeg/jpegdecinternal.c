...
void JpegDecClearStructs(JpegDecContainer * pJpegDecCont)
{
    ...
    pJpegDecCont->info.timeout = (RK_U32) DEC_X170_TIMEOUT_LENGTH;
...
#define PTR_JPGC   pJpegDecCont
    ...
    JPEGDEC_TRACE_INTERNAL((" <%s>_%d INTERNAL: Set Frame width extension PTR_JPGC->info.X = %d\n", __func__, __LINE__, PTR_JPGC->info.X));
    SetDecRegister(PTR_JPGC->jpegRegs, HWIF_PIC_MB_W_EXT,
                   ((((PTR_JPGC->info.X) >> (4)) & 0xE00) >> 9));

    /* frame size, round up the number of mbs */
    JPEGDEC_TRACE_INTERNAL(("INTERNAL: Set Frame width  PTR_JPGC->info.X = %d \n", PTR_JPGC->info.X));
    SetDecRegister(PTR_JPGC->jpegRegs, HWIF_PIC_MB_WIDTH,
                   ((PTR_JPGC->info.X) >> (4)) & 0x1FF);

    /* frame size, round up the number of mbs */
    JPEGDEC_TRACE_INTERNAL(("INTERNAL: Set Frame height extension  PTR_JPGC->info.Y = %d \n", PTR_JPGC->info.Y));
    SetDecRegister(PTR_JPGC->jpegRegs, HWIF_PIC_MB_H_EXT,
                   ((((PTR_JPGC->info.Y) >> (4)) & 0x700) >> 8));

    /* frame size, round up the number of mbs */
    JPEGDEC_TRACE_INTERNAL(("INTERNAL: Set Frame height  PTR_JPGC->info.Y = %d \n", PTR_JPGC->info.Y));
    ...
    JPEGDEC_TRACE_INTERNAL(("INTERNAL: Width should multiple of 16,   <%d, %d>\n", PTR_JPGC->info.fillRight, PTR_JPGC->info.fillBottom));
    ...
    if(PTR_JPGC->info.operationType == JPEGDEC_PROGRESSIVE)
    {
        ...
        if(PTR_JPGC->info.nonInterleaved)
        {
            ...
            {
                coeffBuffer += IOMMUOffsetFix((JPEGDEC_COEFF_SIZE *
        ...
        else
        {
            SetDecRegister(PTR_JPGC->jpegRegs, HWIF_PJPEG_COEFF_BUF,
                           coeffBuffer);
            coeffBuffer += IOMMUOffsetFix((JPEGDEC_COEFF_SIZE) * PTR_JPGC->frame.numBlocks[0]);
            SetDecRegister(PTR_JPGC->jpegRegs, HWIF_PJPEG_DCCB_BASE,
                           coeffBuffer);
            coeffBuffer += IOMMUOffsetFix((JPEGDEC_COEFF_SIZE) * PTR_JPGC->frame.numBlocks[1]);
    ...
    if(PTR_JPGC->info.operationType == JPEGDEC_BASELINE)
    {
        ...
        JPEGDEC_TRACE_INTERNAL(("INTERNAL: Write VLC length amounts to register   I\n"));
        ...
        JPEGDEC_TRACE_INTERNAL(("INTERNAL:  PTR_JPGC->frame.pTableBase = %p\n", &(PTR_JPGC->frame.pTableBase)));
		VPUMemFlush(&(PTR_JPGC->frame.pTableBase));

    }
    else if(PTR_JPGC->info.operationType == JPEGDEC_NONINTERLEAVED)
    {
        /* write "length amounts" */
        JPEGDEC_TRACE_INTERNAL(("INTERNAL: Write VLC length amounts to register   II\n"));
    ...
    {
        /* write "length amounts" */
        JPEGDEC_TRACE_INTERNAL(("INTERNAL: Write VLC length amounts to register   III\n"));
    ...
    /* write table base */
    JPEGDEC_TRACE_INTERNAL(("INTERNAL: Set AC,DC,QP table base address,  PTR_JPGC->frame.pTableBase.phy_addr = %x\n", PTR_JPGC->frame.pTableBase.phy_addr));
    ...
    JPEGDEC_TRACE_INTERNAL(("INTERNAL: PTR_JPGC->frame.Ri = %d\n", PTR_JPGC->frame.Ri));
...
#define PTR_JPGC   pJpegDecCont
    ...
    if(PTR_JPGC->ppInstance != NULL && PTR_JPGC->ppControl.usePipeline == 0)
    {
        ...
        SetDecRegister(PTR_JPGC->jpegRegs, HWIF_DEC_OUT_BASE,
                       PTR_JPGC->asicBuff.outLumaBuffer.phy_addr +
                       IOMMUOffsetFix(PTR_JPGC->info.lumaPos));
        ...
        if(PTR_JPGC->image.sizeChroma)
        {
            ...
            SetDecRegister(PTR_JPGC->jpegRegs, HWIF_JPG_CH_OUT_BASE,
                           PTR_JPGC->asicBuff.outChromaBuffer.phy_addr +
                           IOMMUOffsetFix(PTR_JPGC->info.chromaPos));
        }
...
#define PTR_JPGC   pJpegDecCont
    ...
    /* frame size, round up the number of mbs */
    JPEGDEC_TRACE_INTERNAL(("INTERNAL: Set Frame width extension PTR_JPGC->info.X = %d \n", PTR_JPGC->info.X));
    SetDecRegister(PTR_JPGC->jpegRegs, HWIF_PIC_MB_W_EXT,
                   ((((PTR_JPGC->info.X) >> (4)) & 0xE00) >> 9));
...
#define JPG_STR     pJpegDecCont->stream
    ...
    /* calculate and set stream start address to hw */
    addrTmp = ((RK_U32) JPG_STR.streamBus + IOMMUOffsetFix(((RK_U32) JPG_STR.pStartOfStream & 0x3) +
               (RK_U32) (JPG_STR.pCurrPos - JPG_STR.pStartOfStream))) & (~IOMMUOffsetFix(7));
...
#define PTR_JPGC   pJpegDecCont
    ...
    tmp =
        VPUMallocLinear(&(PTR_JPGC->frame.pTableBase), (sizeof(RK_U8) * tableSize));
    if(tmp == -1)
    {
        return (JPEGDEC_MEMFAIL);
    }
    ...
    JPEGDEC_TRACE_INTERNAL(("ALLOCATE: VLC/QP virtual %x bus %x\n",
                            (RK_U32) PTR_JPGC->frame.pTableBase.vir_addr,
                            PTR_JPGC->frame.pTableBase.phy_addr));
       ...
       (PTR_JPGC->ppInstance != NULL && !PTR_JPGC->ppControl.usePipeline))
    {
        if(PTR_JPGC->info.givenOutLuma.vir_addr == NULL)
        {
            ...
            tmp =
                VPUMallocLinear(&(PTR_JPGC->asicBuff.outLumaBuffer), (PTR_JPGC->image.sizeLuma));
            if(tmp == -1)
            {
                return (JPEGDEC_MEMFAIL);
            }
        ...
        JPEGDEC_TRACE_INTERNAL(("ALLOCATE: Luma virtual %x bus %x\n",
                                (RK_U32) PTR_JPGC->asicBuff.outLumaBuffer.
                                vir_addr,
                                PTR_JPGC->asicBuff.outLumaBuffer.phy_addr));
        ...
        if(PTR_JPGC->image.sizeChroma)
        {
            if(PTR_JPGC->info.givenOutChroma.vir_addr == NULL)
            {
                if(PTR_JPGC->info.operationType != JPEGDEC_BASELINE)
                {
                    tmp =
                        VPUMallocLinear(&(PTR_JPGC->asicBuff.outChromaBuffer),
                                        (PTR_JPGC->image.sizeChroma / 2));
                    if(tmp == -1)
                        return (JPEGDEC_MEMFAIL);
                    tmp =
                        VPUMallocLinear(&(PTR_JPGC->asicBuff.outChromaBuffer2),
                                        (PTR_JPGC->image.sizeChroma / 2));
                    if(tmp == -1)
                        return (JPEGDEC_MEMFAIL);
                }
            ...
            JPEGDEC_TRACE_INTERNAL(("ALLOCATE: Chroma virtual %x bus %x\n",
                                    (RK_U32) PTR_JPGC->asicBuff.outChromaBuffer.
                                    vir_addr,
...
void JpegFlushRegs(JpegDecContainer * pJpegDecCont)
{
	int i;	
/*
	for(i=0 ; i< DEC_X170_REGISTERS+41;i++)
	{
		ALOGE("INTERNAL: REGISTER %d VALUE 0x%08x\n",i,
                            pJpegDecCont->jpegRegs[i]);
	}
printf 60 REGISTER VALUE*/
	if (VPUClientSendReg(pJpegDecCont->socket, pJpegDecCont->jpegRegs, DEC_X170_REGISTERS + 41))
		printf("<%s>_%d  JPEGFlushRegs fail\n", __func__, __LINE__);
	else
		printf("<%s>_%d  JPEGFlushRegs success\n", __func__, __LINE__);
...
static RK_U32 IOMMUOffsetFix(RK_U32 offset)
{
	if(0/*VPUClientGetIOMMUStatus()*/){
        JPEGDEC_TRACE_INTERNAL(("IOMMU is open,phy_addr+(offset << 10)"));
	    return (offset<<10);
	}
	else{
	    JPEGDEC_TRACE_INTERNAL(("IOMMU is close,phy_addr don't change")); 
	    return offset;
	} 
}
...
void JpegDecInitHWEmptyScan(JpegDecContainer * pJpegDecCont, RK_U32 componentId)
{
    ...
    if(componentId)
    {
        coeffBuffer += IOMMUOffsetFix(JPEGDEC_COEFF_SIZE * numBlocks);
