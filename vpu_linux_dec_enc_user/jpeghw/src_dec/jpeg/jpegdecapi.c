...
#include "vpu.h"
#include <utils/Log.h>
#define LOG_TAG "JPEGDEC_API_TRACE"
...
JpegDecRet JpegDecInit(JpegDecInst * pDecInst)
{
    ...
    *pDecInst = NULL;   /* return NULL instance for any error */
	...
	socket = VPUClientInit(VPU_DEC_PP);
	//LOGD("VPUClientInit has done.the socket number is %d\n",socket);
    if(socket <= 0)
    {
        JPEGDEC_API_TRC("JpegDecInit# ERROR: DWL Init failed,socket = %d",socket);
    ...
    for(i = 1; i < DEC_X170_REGISTERS + 41; i++)
    {
        ...
        stream.readBits++)
    {
        ...
        if(JpegDecGetByte(&(stream)) == 0xFF)
        {
            ...
            case DRI:
				...
				JPEGDEC_API_TRC("JpegDecGetImageInfo ERROR: Unsupported DRI.");
                //return (JPEGDEC_UNSUPPORTED);//unsupported, maybe fixed in future
                break;
                            ...
                            stream.readBits++)
                        {
                            ...
                            if(JpegDecGetByte(&(stream)) == 0xFF)
                            {
                                ...
                                case DRI:
                                    ...
                                    appBits += (headerLength * 8);
									JPEGDEC_API_TRC("JpegDecGetImageInfo thumb ERROR: Unsupported DRI.");
                                    //return (JPEGDEC_UNSUPPORTED);//unsupported, maybe fixed in future
                                    break;
                                case APP0:
                                case APP1:
...
#define JPG_INFO  PTR_JPGC->info
    ...
    if(PTR_JPGC->info.progressiveFinish)
    {
		...
		VPU_CMD_TYPE cmd;
		RK_S32 len;
		dwlret = VPUClientWaitResult(PTR_JPGC->socket, PTR_JPGC->jpegRegs, DEC_X170_REGISTERS + 41, &cmd, &len);
    ...
    do
    {
        ...
        if(JpegDecGetByte(&(PTR_JPGC->stream)) == 0xFF)
        {
            ...
            case DHT:
                //JPEGDEC_API_TRC("JpegDecDecode# JpegDecDecodeHuffmanTables dec");
                retCode = JpegDecDecodeHuffmanTables(PTR_JPGC);
                //JPEGDEC_API_TRC("JpegDecDecode# JpegDecDecodeHuffmanTables stops");
                if(retCode != JPEGDEC_OK)
                {
                    if(retCode == JPEGDEC_STRM_ERROR)
                    {
            ...
            case DRI:
				...
				JPEGDEC_API_TRC("JPEGDEC_DECODE ERROR: Unsupported DRI.");
                //return (JPEGDEC_UNSUPPORTED);//hardware unsupported, maybe fixed in future
                break;
        ...
        do
        {
            ...
            if(!PTR_JPGC->info.inputBufferEmpty)
            {
                   ...
                   PTR_JPGC->ppControl.usePipeline == 0)
                {
					...
					printf(" JpegDecSliceSizeCalculation sliceHeight is %d.PTR_JPGC->info.sliceMbSetValue is %d\n",
...
static void JpegDecPreparePp(JpegDecContainer * pJpegDecCont)
{
    ...
    {
        RK_U32 tmp = GetDecRegister(pJpegDecCont->jpegRegs, HWIF_DEC_OUT_ENDIAN);
...
/*------------------------------------------------------------------------------
    5.8. Function name   : jpegRegisterPP
         ...
         Return type     : RK_S32 - return 0 for success or a negative error code
