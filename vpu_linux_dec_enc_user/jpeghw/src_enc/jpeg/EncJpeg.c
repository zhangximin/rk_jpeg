...
RK_U32 EncJpegHdr(stream_s * stream, jpegData_s * data)
{
    ...
    data->frame.Y = (RK_U32) data->height;
    data->frame.X = (RK_U32) data->width;
...
void EncJpegAPP0Header(stream_s * stream, jpegData_s * data)
{
    ...
    if(data->appn.thumbEnable)
    {
        ...
        const RK_U8 * thumbData = (RK_U8*)thumb->data;
        ...
        EncJpegHeaderPutBits(stream, (RK_U32)thumb->format, 8);
...
{
    RK_U32 dc_lum, dc_chrom;
    RK_U32 ac_lum, ac_chrom;
    RK_U32 dc_vij, ac_vij;
