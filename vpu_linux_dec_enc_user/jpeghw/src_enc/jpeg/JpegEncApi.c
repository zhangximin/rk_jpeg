...
--------------------------------------------------------------------------------
    3. Module defines
------------------------------------------------------------------------------*/
#define JPEGENC_TRACE   0
/* Tracing macro */
#if JPEGENC_TRACE
#define APITRACE(str) JpegEnc_Trace(str)
#else
#define APITRACE(str) ALOGD(str)
#endif
...
static RK_S32 CheckThumbnailCfg(const JpegEncThumb * pCfgThumb);
static RK_S32 CheckFullSize(const JpegEncCfg * pCfgFull);
...
//ALOGE("init, pEncInst->inst: %p", pEncInst->inst);
...
JpegEncRet JpegEncRelease(JpegEncInst inst)
{
    ...
    {
	//ALOGE("release, pEncInst->inst: %p, pEncInst: %p",pEncInst->inst, pEncInst);
...
JpegEncRet JpegEncSetThumbnail(JpegEncInst inst, const JpegEncThumb *pJpegThumb)
{
    ...
    (void)memcpy(&pEncInst->jpeg.thumbnail, pJpegThumb, sizeof(JpegEncThumb));
...
//ALOGD("inputwidth: %d, inputheight: %d, codingw: %d, codingh: %d", pEncCfg->inputWidth, pEncCfg->inputHeight,
//	pEncCfg->codingWidth, pEncCfg->codingHeight);
    ...
    mbTotal =
        (RK_U32) (((pEncCfg->codingWidth + 15) / 16) * ((pEncCfg->codingHeight +
                                                      15) / 16));
    ...
    pEncInst->jpeg.restart.Ri = (RK_U32)(pEncCfg->restartInterval * widthMbs);
    ...
    else
    {
        ...
        pEncInst->jpeg.sliceRows = (RK_U32) pEncCfg->restartInterval;
    ...
    if(EncAsicMemAlloc_V2(&pEncInst->asic, (RK_U32) pEncInst->jpeg.width,
...
JpegEncRet JpegEncEncode(JpegEncInst inst, const JpegEncIn * pEncIn,
                         JpegEncOut * pEncOut/*, VPUMemLinear_t outMem*/)
{
    jpegInstance_s *pEncInst = (jpegInstance_s *) inst;
    jpegData_s *jpeg;
    asicData_s *asic;
    ...
    if((jpeg->appn.thumbEnable) &&
       (pEncIn->outBufSize < ((RK_U32)jpeg->thumbnail.dataLength + 1024)))
    {
        APITRACE("JpegEncEncode: ERROR Invalid output buffer");
        return JPEGENC_INVALID_ARGUMENT;
    }
    ...
    if(EncJpegSetBuffer(&pEncInst->stream, (RK_U8 *) pEncIn->pOutBuf,
                        (RK_U32) pEncIn->outBufSize) == ENCHW_NOK)
    {
        APITRACE("JpegEncEncode: ERROR Invalid output buffer");
        return JPEGENC_INVALID_ARGUMENT;
    }
    ...
    asic->regs.pixelsOnRow = (RK_U32) preProcess->lumWidthSrc;
    asic->regs.outputStrmSize = pEncIn->outBufSize;

	printf("asic->regs.outputStrmSize is 0x%x \n",asic->regs.outputStrmSize);
    ...
    if(jpeg->codingType == ENC_WHOLE_FRAME)
    {
        ...
        asic->regs.jpegRestartInterval =
            (jpeg->restart.Ri / ((RK_U32) jpeg->width / 16));
    ...
    if((jpeg->codingType == ENC_PARTIAL_FRAME) &&
       (preProcess->verOffsetSrc >
        (RK_U32) (jpeg->sliceNum * jpeg->sliceRows * 16)))
    {
        jpeg->sliceNum++;
    ...
    else
    {
        ...
        {
            preProcess->lumHeight = (16 * ((RK_S32) pEncInst->jpeg.restart.Ri /
    ...
    if(ret != JPEGENCODE_OK)
    {
        ...
        pEncOut->jfifSize = (RK_S32) pEncInst->stream.byteCnt;
...
/*******************************************************************************
 ...
 Argument      : JpegEncCfg
*******************************************************************************/
RK_S32 CheckFullSize(const JpegEncCfg * pCfgFull)
{
    if((pCfgFull->inputWidth > 8192) || (pCfgFull->inputHeight > 8192))
    {
    	ALOGD("pCfgFull->inputWidth > 8192) || (pCfgFull->inputHeight > 8192");
        return JPEGENC_ERROR;
    }

    if((pCfgFull->codingWidth < 96) || (pCfgFull->codingWidth > (511 * 16)))
    {
    	ALOGD("pCfgFull->codingWidth < 96) || (pCfgFull->codingWidth > (511 * 16)");
        return JPEGENC_ERROR;
    }

    if((pCfgFull->codingHeight < 32) || (pCfgFull->codingHeight > (511 * 16)))
    {
    	ALOGD("pCfgFull->codingHeight < 32) || (pCfgFull->codingHeight > (511 * 16)");
        return JPEGENC_ERROR;
    }
    ...
    {
    	ALOGD("((pCfgFull->codingWidth + 15) >> 4) * ((pCfgFull->codingHeight + 15) >> 4) > JPEGENC_MAX_SIZE");
        return JPEGENC_ERROR;
    }

    if(0&&(pCfgFull->codingWidth & (3)) != 0)
    {
    	ALOGD("(pCfgFull->codingWidth & (3)) != 0");
        return JPEGENC_ERROR;
    }

    if((pCfgFull->codingHeight & (1)) != 0)
    {
    	ALOGD("(pCfgFull->codingHeight & (1)) != 0");
        return JPEGENC_ERROR;
    }

    if((pCfgFull->inputWidth & (15)) != 0)
    {
    	ALOGD("(pCfgFull->inputWidth & (15)) != 0");
        return JPEGENC_ERROR;
    }
/*
...
/*******************************************************************************
 ...
 Argument      : JpegEncCfg
*******************************************************************************/
RK_S32 CheckThumbnailCfg(const JpegEncThumb * pCfgThumb)
{
    RK_U16 dataLength;
...
bool_e CheckJpegCfg(const JpegEncCfg * pEncCfg)
{
	...
	//		usleep(10);
        ...
        {
        	APITRACE("JpegEncInit: EWL_HW_CONFIG_NOT_SUPPORTED");
            return ENCHW_NOK;
        }
        if(cfg.rgbEnabled == EWL_HW_CONFIG_NOT_SUPPORTED &&
           pEncCfg->frameType > 3)
        {
        	APITRACE("JpegEncInit: EWL_HW_CONFIG_NOT_SUPPORTED, pEncCfg->frameType > 3");
            return ENCHW_NOK;
        }
    }
